...
DWORD WINAPI thrFunc(void * param){
...
void Mul_Threading(double* a, double* b, double* c, int k, int m, int n, int np){
	...
	for (int i = 0; i < np; i++) {
		...
		par->m=m;
		par->n=n;
		hands[i] = CreateThread(NULL, 0, &thrFunc, par, 0, NULL);
	}
	//WaitForMultipleObjects(np, hands, true, INFINITE);
	for (int i = 0; i < np; i++) {
		bool next=false;
		while (!next){
			unsigned long r=WaitForSingleObject(hands[i],100);
			if (r==WAIT_OBJECT_0){
				next=true;
			}
		}
...
DWORD WINAPI thrFunc2(void * param){
	int bi, r, c, i, w;
	double* temp;
	double el, e, a1, b1;
	double z; z = 0;
...
bool GetLU_Threading(double* lu, int* p, int n, int np){
	...
	for (int i = 0; i < np; i++) {
		...
		par[i]->np=np;
		par[i]->stop=true;
		hands[i] = CreateThread(NULL, 0, &thrFunc2, par[i], 0, NULL);
	}
